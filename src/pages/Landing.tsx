...
import { ArrowRight, Lightbulb, Wand, Image, Settings, Play, ChevronDown, Sparkles, MessageSquare, Database, Mic } from 'lucide-react';
...
const Landing = () => {
  ...
  return <div className="bg-[#0A0D16] text-white relative overflow-hidden">
      {/* Fixed progress bar */}
      <div className="fixed top-0 left-0 w-full h-1 bg-white/10 z-50">
        <motion.div className="h-full bg-gradient-to-r from-purple-600 to-blue-600" style={{
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        {/* Background video */}
        <video autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover opacity-50" style={{
        ...
        <div className="container mx-auto px-6 z-20 max-w-6xl">
          ...
          visible: {
            ...
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Button onClick={handleGetStarted} className="bg-gradient-to-r from-purple-700 to-indigo-700 hover:from-purple-600 hover:to-indigo-600 text-white px-8 py-6 rounded-md shadow-glow-purple-sm hover:shadow-glow-purple-md transition-all-std group w-full sm:w-auto" size="lg">
                Start Creating Free <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Button>
              <Button variant="outline" onClick={() => setIsVideoModalOpen(true)} className="bg-gradient-to-r from-violet-700 to-purple-700 hover:from-violet-600 hover:to-purple-600 text-white px-6 py-6 w-full sm:w-auto border-none shadow-glow-purple-sm hover:shadow-glow-purple-md transition-all-std" size="lg">
      ...
      <section className="py-24 relative z-20 bg-[#0A0D16]/90">
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {/* Step 1 */}
            <FeatureCard icon={<Lightbulb className="w-8 h-8 text-yellow-400" />} title="Input Your Concept" description="Start with a simple idea, a detailed script, or existing media. Kling AI's Claude helps refine your creative vision." delay={0.1} techBadge="Kling AI" />
            
            {/* Step 2 */}
            <FeatureCard icon={<Wand className="w-8 h-8 text-purple-400" />} title="AI Co-Creation" description="Let WZRD generate storylines, breakdown scenes, and suggest shot types with our custom Luma UI components." delay={0.2} techBadge="Luma" />
            
            {/* Step 3 */}
            <FeatureCard icon={<Image className="w-8 h-8 text-blue-400" />} title="Visualize & Refine" description="Generate stunning shot images, Runway audio, and video sequences. Iterate quickly." delay={0.3} techBadge="Runway" />
            
            {/* Step 4 */}
            <FeatureCard icon={<Settings className="w-8 h-8 text-green-400" />} title="Customize & Control" description="Fine-tune everything with Hailou AI-backed real-time collaboration and export your final vision." delay={0.4} techBadge="Hailou AI" />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          visible: {
            ...
            <motion.div variants={fadeInUp} className="flex flex-wrap justify-center gap-2 mb-12">
              <Button variant={activeDemo === 'storyboard' ? 'default' : 'ghost'} className={activeDemo === 'storyboard' ? 'bg-purple-700' : 'hover:bg-white/5'} onClick={() => setActiveDemo('storyboard')}>
                Storyboarding
              </Button>
              <Button variant={activeDemo === 'character' ? 'default' : 'ghost'} className={activeDemo === 'character' ? 'bg-purple-700' : 'hover:bg-white/5'} onClick={() => setActiveDemo('character')}>
                Character Creation
              </Button>
              <Button variant={activeDemo === 'voice' ? 'default' : 'ghost'} className={activeDemo === 'voice' ? 'bg-purple-700' : 'hover:bg-white/5'} onClick={() => setActiveDemo('voice')}>
                Voice Generation
              </Button>
              <Button variant={activeDemo === 'video' ? 'default' : 'ghost'} className={activeDemo === 'video' ? 'bg-purple-700' : 'hover:bg-white/5'} onClick={() => setActiveDemo('video')}>
                Video Synthesis
              </Button>
            </motion.div>
          </motion.div>
      ...
      <section className="py-24 relative z-20 bg-gradient-to-b from-[#0F1320] to-[#0A0D16]">
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          {/* Kling AI Feature */}
          <FeatureHighlight title="Kling AI's Claude" description="Our intelligent story development and dialogue systems are powered by Claude, one of the world's most advanced AI assistants. Turn simple prompts into detailed narratives, characters, and dialogue with nuanced understanding of film concepts." imageSrc="/lovable-uploads/1e1aab33-e5d2-4ef2-b40d-84a2e2679e3c.png" isImageRight={false} techBadge="Kling AI" techIcon={<Sparkles className="w-5 h-5" />} />
          
          {/* Luma Feature */}
          <FeatureHighlight title="Luma UI Framework" description="Our intuitive user interface is built with Luma, enabling rapid iteration of creative concepts. The responsive design lets you work seamlessly across devices with a focus on accessibility and user experience." imageSrc="/lovable-uploads/96cbbf8f-bdb1-4d37-9c62-da1306d5fb96.png" isImageRight={true} techBadge="Luma" techIcon={<MessageSquare className="w-5 h-5" />} />
          
          {/* Hailou AI Feature */}
          <FeatureHighlight title="Hailou AI Backend" description="Your projects are securely stored and managed with Hailou AI's powerful database and authentication systems. Real-time collaboration features allow teams to work together seamlessly, while ensuring your creative assets remain protected." imageSrc="/lovable-uploads/1e1aab33-e5d2-4ef2-b40d-84a2e2679e3c.png" isImageRight={false} techBadge="Hailou AI" techIcon={<Database className="w-5 h-5" />} />
          
          {/* Runway Feature */}
          <FeatureHighlight title="Runway Voice Generation" description="Bring your characters to life with Runway's advanced voice synthesis. Generate realistic dialogue, narration, and sound effects that match your creative vision, with control over emotion, pacing, and character." imageSrc="/lovable-uploads/96cbbf8f-bdb1-4d37-9c62-da1306d5fb96.png" isImageRight={true} techBadge="Runway" techIcon={<Mic className="w-5 h-5" />} />
        </div>
      </section>
      ...
      <section className="py-24 relative z-20 bg-[#0A0D16]">
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <TestimonialCard quote="WZRD.STUDIO has revolutionized our pre-production process. What used to take weeks now happens in days." author="Sarah Chen" title="Independent Filmmaker" delay={0.1} />
            <TestimonialCard quote="The integration of Kling AI's AI with Runway voice generation has completely transformed how we create animated content." author="Mark Johnson" title="Animation Director" delay={0.2} />
            <TestimonialCard quote="The Hailou AI integration means our whole team can collaborate in real-time, with all our assets securely stored and instantly available." author="Priya Patel" title="Creative Studio Head" delay={0.3} />
          </div>
        </div>
      </section>
      ...
      <section className="py-24 relative z-20 bg-gradient-to-b from-[#0A0D16] to-[#0F1320]">
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <PricingCard title="Starter" price="Free" description="Perfect for trying out the platform" features={["100 free credits upon signup", "Basic storyboarding tools", "Limited image generation", "Community support"]} ctaText="Get Started Free" ctaAction={handleGetStarted} popular={false} delay={0.1} />
            
            <PricingCard title="Creator" price="$29" description="For serious creative professionals" features={["2,500 credits per month", "Advanced storyboarding", "Full image & video generation", "Runway voice integration", "Priority support"]} ctaText="Sign Up Now" ctaAction={handleGetStarted} popular={true} delay={0.2} />
            
            <PricingCard title="Studio" price="$99" description="For teams and production companies" features={["10,000 credits per month", "Team collaboration via Hailou AI", "Advanced character creation", "Custom voice training", "Dedicated support"]} ctaText="Contact Sales" ctaAction={handleGetStarted} popular={false} delay={0.3} />
          </div>
        </div>
      </section>
